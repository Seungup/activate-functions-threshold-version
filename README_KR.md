...
해당 형식은 곡선이 X좌표보다 위에 존재하게 되며, 연속함과 동시에

**( 입력값 * 0.001 )** 에 대하여  [Leaky ReLU](https://arxiv.org/abs/1505.00853)의 특성과 유사하기 때문에


**{알파 * 지수함수(임계값 - 입력값)} ~= ( 입력값 * 0.001)** 라고도 할 수 있습니다.

**하지만 그대로 사용하면 X좌표 위에 존재하기 때문에 입력값이 0보다 작을때와 동일하게 음수로 나타내기 위해서 -1을 곱해주야 하지만 값은 0과 매우 근접하기에 부호를 사실상 무시해도 상관은 없습니다.
